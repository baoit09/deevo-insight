...
//var chaincodeid = 'mychaincode2';
var channel_name = process.argv[2];
var chaincode_id = process.argv[3];
...
test('\n\n***** clean up the connection profile testing stores  *****\n\n', function(t) {
	...
	client.loadFromConfig('test/fixtures/org3.yaml');
	client_config = client.getClientConfig();

	store_path = client_config.credentialStore.path;
	logger.debug('removing org3 credentialStore %s',store_path);
	fsx.removeSync(store_path);

	crypto_path = client_config.credentialStore.cryptoStore.path;
	logger.debug('removing org3 cryptoStore %s',crypto_path);
	fsx.removeSync(crypto_path);
	
	client.loadFromConfig('test/fixtures/org4.yaml');
	client_config = client.getClientConfig();

	store_path = client_config.credentialStore.path;
	logger.debug('removing org4 credentialStore %s',store_path);
	fsx.removeSync(store_path);

	crypto_path = client_config.credentialStore.cryptoStore.path;
	logger.debug('removing org4 cryptoStore %s',crypto_path);
	fsx.removeSync(crypto_path);
	
	client.loadFromConfig('test/fixtures/org5.yaml');
	client_config = client.getClientConfig();

	store_path = client_config.credentialStore.path;
	logger.debug('removing org5 credentialStore %s',store_path);
	fsx.removeSync(store_path);

	crypto_path = client_config.credentialStore.cryptoStore.path;
	logger.debug('removing org5 cryptoStore %s',crypto_path);
	fsx.removeSync(crypto_path);
...
test('\n\n***** use the connection profile file  *****\n\n', function(t) {
	...
	var client_org3 = Client.loadFromConfig('test/fixtures/network.yaml');
	var client_org4 = Client.loadFromConfig('test/fixtures/network.yaml');
	var client_org5 = Client.loadFromConfig('test/fixtures/network.yaml');
	...
	var channel_on_org3 = null;
	var channel_on_org4 = null;
	var channel_on_org5 = null;
	...
	client_org3.loadFromConfig('test/fixtures/org3.yaml');
	client_org4.loadFromConfig('test/fixtures/org4.yaml');
	client_org5.loadFromConfig('test/fixtures/org5.yaml');
	...
	.then((nothing) => {
		...
		t.equals(caService._fabricCAClient._caName,'rca.org1.deevo.com', 'checking that caname is correct after resetting the config');
		...
		t.equals(caService._fabricCAClient._caName,'rca.org2.deevo.com', 'checking that caname is correct after resetting the config');
		let request = {
		...
		return client_org3.initCredentialStores();
	}).then ((nothing) => {
		t.pass('Successfully created the key value store and crypto store based on the sdk config and connection profile');

		// get the CA associated with this client's organization
		let caService = client_org3.getCertificateAuthority();
		t.equals(caService._fabricCAClient._caName,'rca.org3.deevo.com', 'checking that caname is correct after resetting the config');
		let request = {
			enrollmentID: 'admin-org3',
			enrollmentSecret: 'admin-org3pw',
			profile: 'tls'
		};
		return caService.enroll(request);
	}).then((enrollment) => {
		...
		// set the material on the client to be used when building endpoints for the user
		client_org3.setTlsClientCertAndKey(cert, key);

		// tell this client instance where the state and key stores are located
		return client_org4.initCredentialStores();
	}).then((nothing) => {
		t.pass('Successfully created the key value store and crypto store based on the sdk config and connection profile');

		// get the CA associated with this client's organization
		let caService = client_org4.getCertificateAuthority();
		t.equals(caService._fabricCAClient._caName,'rca.org4.deevo.com', 'checking that caname is correct after resetting the config');
		let request = {
			enrollmentID: 'admin-org4',
			enrollmentSecret: 'admin-org4pw',
			profile: 'tls'
		};
		return caService.enroll(request);
	}).then((enrollment) => {
		...
		// set the material on the client to be used when building endpoints for the user
		client_org4.setTlsClientCertAndKey(cert, key);

		// tell this client instance where the state and key stores are located
		return client_org5.initCredentialStores();
	}).then((nothing) => {
		t.pass('Successfully created the key value store and crypto store based on the sdk config and connection profile');

		// get the CA associated with this client's organization
		let caService = client_org5.getCertificateAuthority();
		t.equals(caService._fabricCAClient._caName,'rca.org5.deevo.com', 'checking that caname is correct after resetting the config');
		let request = {
			enrollmentID: 'admin-org5',
			enrollmentSecret: 'admin-org5pw',
			profile: 'tls'
		};
		return caService.enroll(request);
	}).then((enrollment) => {
		...
		// set the material on the client to be used when building endpoints for the user
		client_org5.setTlsClientCertAndKey(cert, key);
		...
		// sign the config by admin from org2
		let signature2 = client_org2.signChannelConfig(config);
		let string_signature2 = signature2.toBuffer().toString('hex');
		t.pass('Successfully signed config update for org2');

		// collect the signature from org2's admin
		signatures.push(string_signature2);
		
		// sign the config by admin from org2
		let signature3 = client_org3.signChannelConfig(config);
		let string_signature3 = signature3.toBuffer().toString('hex');
		t.pass('Successfully signed config update for org3');

		// collect the signature from org2's admin
		signatures.push(string_signature3);
		
		// sign the config by admin from org2
		let signature4 = client_org4.signChannelConfig(config);
		let string_signature4 = signature4.toBuffer().toString('hex');
		t.pass('Successfully signed config update for org4');

		// collect the signature from org2's admin
		signatures.push(string_signature4);
		
		// sign the config by admin from org2
		let signature5 = client_org5.signChannelConfig(config);
		//let string_signature5 = signature5.toBuffer().toString('hex');
		t.pass('Successfully signed config update for org5');

		// collect the signature from org2's admin
		signatures.push(signature5);
		...
		let request = {
			...
			signatures : signatures,
			name : channel_name,
			orderer : 'orderer0.org0.deevo.com', //this assumes we have loaded a connection profile
			txId  : tx_id
		};
	...
	.then((result) => {
		...
		channel_on_org3 = client_org3.getChannel(channel_name);
		channel_on_org4 = client_org4.getChannel(channel_name);
		channel_on_org5 = client_org5.getChannel(channel_name);
		...
		let request = {
			targets: ['peer0.org1.deevo.com'], // this does assume that we have loaded a
		...
		let tx_id = client_org3.newTransactionID(true);
		let request = {
			targets: ['peer0.org3.deevo.com'], // this does assume that we have loaded a
			                                     // connection profile with a peer by this name
			block : genesis_block,
			txId : 	tx_id
		};

		return channel_on_org3.joinChannel(request);
	}).then((results) => {
		
		logger.debug(util.format('Join Channel R E S P O N S E  for a string target: %j', results));
		// second of the results should have good status
		if(results && results[1] && results[1].response && results[1].response.status == 200) {
			t.pass(util.format('Successfully had peer in organization %s join the channel', 'org3'));
		} else {
			t.fail(' Failed to join channel');
			throw new Error('Failed to join channel');
		}
		
		let tx_id = client_org4.newTransactionID(true);
		let request = {
			targets: ['peer0.org4.deevo.com'], // this does assume that we have loaded a
			                                     // connection profile with a peer by this name
			block : genesis_block,
			txId : 	tx_id
		};

		return channel_on_org4.joinChannel(request);
	}).then((results) => {
		
		logger.debug(util.format('Join Channel R E S P O N S E  for a string target: %j', results));
		// second of the results should have good status
		if(results && results[1] && results[1].response && results[1].response.status == 200) {
			t.pass(util.format('Successfully had peer in organization %s join the channel', 'org4'));
		} else {
			t.fail(' Failed to join channel');
			throw new Error('Failed to join channel');
		}
		
		let tx_id = client_org5.newTransactionID(true);
		let request = {
			targets: ['peer0.org5.deevo.com'], // this does assume that we have loaded a
			                                     // connection profile with a peer by this name
			block : genesis_block,
			txId : 	tx_id
		};

		return channel_on_org5.joinChannel(request);
	}).then((results) => {
		logger.debug(util.format('Join Channel R E S P O N S E  for a string target: %j', results));

		if(results && results[0] && results[0].response && results[0].response.status == 200) {
			t.pass(util.format('Successfully had peer in organization %s join the channel', 'org5'));
		} else {
			t.fail(' Failed to join channel on org5');
			throw new Error('Failed to join channel on org5');
		}
		return sleep(10000);
	}).then(()=>{
		...
		let tx_id = client_org3.newTransactionID(true); // be sure to get a admin transaction ID
		// send proposal to endorser
		let request = {
			targets: ['peer0.org3.deevo.com'],
			chaincodePath: 'github.com/example_cc',
			chaincodeId: chaincode_id,
			chaincodeVersion: 'v1',
			chaincodePackage: '',
		...
		return client_org3.installChaincode(request);
	}).then((results) => {
		if(results && results[0] && results[0][0].response && results[0][0].response.status == 200) {
			t.pass('Successfully installed chain code on org3');
		} else {
			t.fail(' Failed to install chaincode on org1');
			throw new Error('Failed to install chain code on org3');
		}

		let tx_id = client_org4.newTransactionID(true); // be sure to get a admin transaction ID
		// send proposal to endorser
		let request = {
			targets: ['peer0.org4.deevo.com'],
			chaincodePath: 'github.com/example_cc',
			chaincodeId: chaincode_id,
			chaincodeVersion: 'v1',
			chaincodePackage: '',
		...
		return client_org4.installChaincode(request);
	}).then((results) => {
		if(results && results[0] && results[0][0].response && results[0][0].response.status == 200) {
			t.pass('Successfully installed chain code on org4');
		} else {
			t.fail(' Failed to install chaincode on org4');
			throw new Error('Failed to install chain code on org4');
		}

		let tx_id = client_org5.newTransactionID(true); // be sure to get a admin transaction ID
		// send proposal to endorser
		let request = {
			targets: ['peer0.org5.deevo.com'],
			chaincodePath: 'github.com/example_cc',
			chaincodeId: 'example',
			chaincodeVersion: 'v1',
			chaincodePackage: '',
		...
		return client_org5.installChaincode(request);
	})
	.then((results) => {
		if(results && results[0] && results[0][0].response && results[0][0].response.status == 200) {
			t.pass('Successfully installed chain code on org2');
		...
		return client_org1.setUserContext({username:'admin-org1', password:'admin-org1pw'});
	}).then((admin) => {
		...
		let eventhub = channel_on_org1.newChannelEventHub('peer0.org1.deevo.com');
		...
		return new Promise((resolve, reject) => {
			...
			// listeners.
			let channel_event_hub = channel_on_org1.newChannelEventHub('peer0.org1.deevo.com');
		...
		return new Promise((resolve, reject) => {
			...
			// we should have the an channel event hub defined on the "peer0.org1.deevo.com"
			...
			t.equals(channel_event_hub.getPeerAddr(),'peer0.org1.deevo.com:7051',' channel event hub address ');
		...
		return client_org1.queryChannels('peer0.org1.deevo.com');
	}).then((results) => {
		...
		return client_org1.queryInstalledChaincodes('peer0.org1.deevo.com', true); // use admin
	}).then((results) => {
